...
func Login_page_get(ctx *fasthttp.RequestCtx) {
	...
	err := mongodb.Collection_cookie.FindOne(context.TODO(), filter).Decode(&cookiehandle)
	if err == nil {
...
func Login_page_post(ctx *fasthttp.RequestCtx) {
	...
	if encryption.CheckPassowrd(password, res.Salt, res.Iterations, res.PasswordHashed) {
		...
		_, err = mongodb.Collection_cookie.InsertOne(context.TODO(), define.DB_Cookie{UserID: res.ID, Time: primitive.NewDateTimeFromTime(time.Now()), Cookie: cookieid})
		if err != nil {
			panic(err)
		}
